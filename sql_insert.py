...
def insert(key,mesbody):
    ...
    sql="insert into " +TABLE+ "(channel_id,channel,title,url,times,mesbody,level,site,is_keywords,keywords_id) values(9,'实时推特',%s,NULL,%s,NULL,2,%s,'N',0)"
    ...
    querySql = 'select * from ' + TABLE + ' where title="' + mesbody + '" and channel_id=9' +  ' order by  id  desc limit 1'
    try:
        # # 翻译成中文
